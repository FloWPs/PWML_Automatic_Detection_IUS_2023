...
from utils.dataset import PWMLDataset
...
class TrainTestPipe:
    ...
    def __load_dataset(self, path, train=False):
        ...
        set = PWMLDataset(path, transform)
        loader = DataLoader(set, batch_size=cfg.batch_size, shuffle=shuffle, drop_last=droplast) # MODIF FLORA
    ...
    def train(self):
        ...
        for epoch in range(cfg.epoch):
            with tqdm(total=len(self.train_loader) + len(self.test_loader)) as t:
                ...
                self.transunet.scheduler.step(test_loss)
